					...
					success:function(res){
						...
						if(res ==1){
							$('.response').html("<div class  = 'container success' > <img src = 'style/images/agt_action_success.png'> <h4> File Uploaded</h4>");
							 setTimeout(function(){ 
								 $('.resume_upload').html("<div class = 'container keywords'> <input class = 'keywords_input'></input> <button class = 'btn btn-info add_keywords'>Add Keyword</button></div>")
							}, 1000); 
						}
						if(res == 0){
							$('.response').html ("<div class  = 'container error' > <img src = 'style/images/error_warning_alert_attention-512.png'> <h4> Invalid File type. Allowd types are: doc(x),pdf,txt</h4>")
						}
						if(res == 2){
